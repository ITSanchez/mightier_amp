...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  if (self.centralManager == nil) {
    NSLog(@"[MightierBLE] initializing CBCentralManager (triggers Bluetooth permission if needed)");
  ...
  if ([@"state" isEqualToString:call.method]) {
    ...
    result([[NSHost currentHost] localizedName] ?: @"macOS");
  } else if([@"startScan" isEqualToString:call.method]) {
    NSLog(@"[MightierBLE] native startScan");
    ...
    @try {
      ...
      NSLog(@"[MightierBLE] connectPeripheral %@", remoteId);
    ...
    @try {
      ...
      NSLog(@"[MightierBLE] discoverServices %@", remoteId);
    ...
    @try {
      ...
      NSLog(@"[MightierBLE] writeCharacteristic uuid=%@ bytes=%lu withoutResponse=%d",
    ...
    @try {
      ...
      NSLog(@"[MightierBLE] setNotifyValue=%d characteristic=%@",
            [request enable],
            [characteristic.UUID fullUUIDString]);
...
- (void)centralManagerDidUpdateState:(nonnull CBCentralManager *)central {
  NSLog(@"[MightierBLE] centralManagerDidUpdateState: %ld", (long)central.state);
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary<NSString *,id> *)advertisementData RSSI:(NSNumber *)RSSI {
  NSLog(@"[MightierBLE] didDiscoverPeripheral name=%@ uuid=%@ rssi=%@",
        peripheral.name ?: advertisementData[CBAdvertisementDataLocalNameKey],
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
  NSLog(@"[MightierBLE] didConnectPeripheral uuid=%@", [[peripheral identifier] UUIDString]);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
  NSLog(@"[MightierBLE] didDisconnectPeripheral uuid=%@ error=%@", [[peripheral identifier] UUIDString], error);
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
  NSLog(@"[MightierBLE] didFailToConnectPeripheral uuid=%@ error=%@", [[peripheral identifier] UUIDString], error);
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
  NSLog(@"[MightierBLE] didDiscoverServices error=%@", error);
  ...
  for(CBService *s in [peripheral services]) {
    NSLog(@"[MightierBLE] Found service: %@", [s.UUID UUIDString]);
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
  NSLog(@"[MightierBLE] didDiscoverCharacteristicsForService %@ error=%@", [service.UUID UUIDString], error);
